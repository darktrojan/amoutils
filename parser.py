...
# This is the same as parser.py from mozilla-central, except that
# XML entities are replaced in DTDParser.
...
class Entity(object):
    ...
    def __init__(self, contents, pp,
                 ...
                 key_span, val_span, post_span, is_dtd=False):
    ...
    def get_val(self):
        ...
        s = self.pp(self.contents[self.val_span[0]:self.val_span[1]])
        if self.is_dtd:
            s = s.replace('&apos;', "'")
            s = s.replace('&quot;', '"')
            s = s.replace('&lt;', '<')
            s = s.replace('&gt;', '>')
            s = s.replace('&amp;', '&')
            return re.sub('&#(x)?([0-9a-fA-F]+);', self.deref, s)
        return s
